...
const Bookshelf = () => {
	...
	const handleEditShelf = (id) => {
		...
		if (
			newName &&
			newName.trim() !== "" &&
			newName.trim() !== currentShelf?.shelfName
		) {
	...
	const handleDeleteShelf = (id) => {
		...
		if (
			currentShelf &&
			window.confirm(
				`Вы уверены, что хотите удалить полку "${currentShelf.shelfName}"?`,
			)
		) {
			deleteShelf(id);
		}
				...
				<main className="bookshelf-content">
					<div className="content-header">
						<h2 className="shelf-title">
							{shelves.find((s) => s.id === activeShelfId)?.shelfName ||
								"Полка не выбрана"}
						</h2>
						{activeShelfId && (
							<button
				...
				<BookModal
					...
					onUpdateBook={async (updatedData) => {
						if (updatedData.title !== selectedBook.title) {
							const updatedBook = await renameBook(
								selectedBook.id,
								updatedData.title,
							);
